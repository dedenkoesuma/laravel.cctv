...
<div class="flex min-h-screen">
    ...
    <div class="flex-1 p-6">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-xl font-medium text-gray-800">Uang Masuk</h1>
                <p class="text-sm text-gray-500">Catat semua pemasukan kas toko</p>
            </div>
        ...
        @endif
        @if(session('error'))
            <div class="bg-amber-100 text-amber-700 px-4 py-3 rounded-lg mb-4 text-sm">
                {{ session('error') }}
            </div>
        @endif
        ...
        <div class="grid grid-cols-3 gap-4 mb-6">
            <div class="bg-white rounded-xl border p-4">
                ...
                <p class="text-2xl font-medium text-green-600">Rp {{ number_format($summary['bulan_ini'], 0, ',', '.') }}</p>
            </div>
            <div class="bg-white rounded-xl border p-4">
                <p class="text-xs text-gray-500">Total hari ini</p>
                <p class="text-2xl font-medium text-green-600">Rp {{ number_format($summary['hari_ini'], 0, ',', '.') }}</p>
            </div>
            <div class="bg-white rounded-xl border p-4">
        ...
        {{-- Filter --}}
        <form method="GET" action="{{ route('uang-masuk.index') }}" class="flex gap-3 mb-4 flex-wrap">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($transaksi as $t)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3">
                                <span class="px-2 py-1 rounded text-xs bg-green-100 text-green-700">{{ $t->kategori }}</span>
                                @if($t->otomatis)
                                    <span class="px-2 py-1 rounded text-xs bg-indigo-100 text-indigo-700 ml-1">Otomatis</span>
                                @endif
                            </td>
                            <td class="px-4 py-3">{{ $t->keterangan }}</td>
                            <td class="px-4 py-3 font-medium text-green-600">+ {{ $t->jumlah_rupiah }}</td>
                            <td class="px-4 py-3">
                                @if($t->otomatis)
                                    <span class="text-gray-300 text-xs" title="Dibuat otomatis dari Pesanan Online">
                                        Terkunci
                                    </span>
                                @else
