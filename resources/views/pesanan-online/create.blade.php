    ...
    <main class="flex-1 min-w-0 p-6 lg:p-8 bg-[#0B0F19] overflow-y-auto">
        ...
        <div class="mb-6 pb-6 border-b border-white/10">
            <div class="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-slate-500 mb-2">
                ...
                <span>/</span>
                <span class="text-slate-300">Buat Pesanan Baru</span>
            </div>
            <h1 class="text-2xl font-bold font-display text-white tracking-tight">Input Pesanan Online</h1>
            <p class="text-sm text-slate-400 mt-1">Catat transaksi pemesanan dari kanal online / marketplace</p>
        </div>
            ...
            <form method="POST" action="{{ route('pesanan-online.store') }}" class="space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="pelanggan" value="{{ old('pelanggan') }}" placeholder="Nama lengkap pelanggan"
                ...
                <div>
                    ...
                    <p class="text-[11px] text-slate-500 mt-2">
                        Centang tipe kertas yang dipakai, lalu masukkan jumlah lembar masing-masing. Dapat memilih lebih dari satu tipe kertas.
                    </p>
                ...
                <div>
                    <label class="flex items-center gap-2.5 text-sm text-slate-300 bg-[#1A2234] border border-white/10 rounded-xl px-4 py-3 w-fit cursor-pointer hover:border-indigo-500/40 transition">
                        <input type="checkbox" name="jasa_potong" value="1" {{ old('jasa_potong') ? 'checked' : '' }}
                ...
                <div>
                    ...
                    <textarea name="catatan" rows="3" placeholder="Informasi ukuran khusus, laminasi, atau catatan dari pembeli..."
                              class="w-full bg-[#1A2234] border border-white/10 text-white placeholder-slate-500 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">{{ old('catatan') }}</textarea>
                </div>
