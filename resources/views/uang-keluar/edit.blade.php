...
<head>
    ...
    <title>Edit Uang Keluar — Toko Print</title>
...
<div class="flex min-h-screen">
    ...
    <main class="flex-1 min-w-0 p-6 lg:p-8 bg-[#0B0F19] overflow-y-auto">
        ...
        <div class="mb-6 pb-6 border-b border-white/10">
            ...
            <h1 class="text-2xl font-bold font-display text-white tracking-tight">Perbarui Uang Keluar</h1>
            <p class="text-sm text-slate-400 mt-1">Ubah rincian data pengeluaran yang telah dicatat</p>
        </div>
        ...
        <div class="max-w-2xl glass-card rounded-2xl p-6 lg:p-8">
            ...
            <form method="POST" action="{{ route('uang-keluar.update', $transaksi->id) }}" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea name="catatan" rows="3" placeholder="Informasi pendukung, nomor nota, atau keterangan tambahan..."
                              class="w-full bg-[#1A2234] border border-white/10 text-white placeholder-slate-500 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition">{{ old('catatan', $transaksi->catatan) }}</textarea>
                </div>
