...
<head>
    ...
    <title>Pesanan Offline — Toko Print</title>
...
<div class="flex min-h-screen">
    ...
    <main class="flex-1 min-w-0 p-6 lg:p-8 bg-[#0B0F19] overflow-y-auto">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 mb-6 border-b border-white/10">
            <div>
                <div class="flex items-center gap-2 text-xs font-semibold tracking-wider uppercase text-slate-500 mb-1.5">
                    ...
                    <span>Toko Print · Transaksi Offline (Walk-in)</span>
                </div>
                <h1 class="text-2xl font-bold font-display text-white tracking-tight">Pesanan Offline</h1>
                <p class="text-sm text-slate-400 mt-0.5">Kelola pesanan dari pelanggan yang datang langsung ke toko</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('pesanan-offline.create') }}"
                   class="inline-flex items-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-700 hover:from-blue-500 hover:to-indigo-600 text-white font-semibold px-4 py-2.5 rounded-xl text-sm transition-all shadow-lg shadow-blue-900/30 hover:shadow-blue-700/40 hover:-translate-y-0.5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-7">
            ...
            <div class="glass-card rounded-2xl p-5 relative overflow-hidden group hover:border-blue-500/40 transition">
                ...
                <div class="flex items-center justify-between mb-2">
                    ...
                    <span class="p-2 rounded-lg bg-blue-500/10 text-blue-400 border border-blue-500/20">
                ...
                <div class="text-[11px] text-slate-500 mt-2 flex items-center gap-1.5">
                    <span class="text-blue-400 font-medium">Bulan berjalan</span>
                </div>
            </div>
        ...
        <div class="glass-card rounded-2xl p-4 mb-6">
            <form method="GET" action="{{ route('pesanan-offline.index') }}" class="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
                <div class="relative flex-1 min-w-[200px]">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                           ...
                           class="w-full bg-[#1A2234] border border-white/10 text-white placeholder-slate-500 text-sm rounded-xl pl-10 pr-4 py-2.5 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition">
                </div>

                <div class="min-w-[190px]">
                    <select name="status" class="w-full bg-[#1A2234] border border-white/10 text-slate-200 text-sm rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition">
                ...
                <div class="flex items-center gap-2">
                    <button type="submit"
                            class="inline-flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 text-white font-medium px-4 py-2.5 rounded-xl text-sm transition">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    ...
                    <tbody class="divide-y divide-white/5">
                        @forelse($pesanan as $p)
                            <tr class="hover:bg-white/[0.025] transition-colors">
                                <td class="px-5 py-4 whitespace-nowrap font-mono font-semibold text-blue-400 text-xs">
                                    {{ $p->no_order }}
                                </td>
                                ...
                                <td class="px-5 py-4 whitespace-nowrap text-center">
                                    <div class="inline-flex items-center gap-1.5">
                                        {{-- Detail --}}
                                        <a href="{{ route('pesanan-offline.show', $p->id) }}"
                                        ...
                                        {{-- Toggle Status --}}
                                        <form method="POST" action="{{ route('pesanan-offline.status', $p->id) }}" class="inline">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit"
                                                    class="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-medium bg-blue-500/10 hover:bg-blue-500/20 text-blue-300 border border-blue-500/20 transition"
                                                    title="Ubah Status">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                        ...
                                        {{-- Edit --}}
                                        <a href="{{ route('pesanan-offline.edit', $p->id) }}"
                                           class="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-medium bg-white/5 hover:bg-white/10 text-slate-300 border border-white/10 transition"
                                           title="Edit Pesanan">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                        ...
                                        {{-- Hapus --}}
                                        <form method="POST" action="{{ route('pesanan-offline.destroy', $p->id) }}"
                            ...
                            <tr>
                                <td colspan="8" class="px-5 py-14 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-slate-300 font-medium text-sm">Tidak ada pesanan offline ditemukan</p>
                                        <p class="text-slate-500 text-xs mt-1">Silakan gunakan tombol "Buat Pesanan" di atas untuk menambahkan data walk-in baru.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
