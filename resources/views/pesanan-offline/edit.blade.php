...
<div class="flex min-h-screen">
    ...
    <div class="flex-1 p-6 max-w-2xl">
        <div class="mb-6">
            <h1 class="text-xl font-medium text-gray-800">Edit Pesanan</h1>
            <p class="text-sm text-gray-500">No. Order: <span class="font-medium text-indigo-600">{{ $pesanan->no_order }}</span></p>
        </div>
        ...
        <form method="POST" action="{{ route('pesanan-offline.update', $pesanan->id) }}" class="bg-white rounded-xl border p-6 space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="catatan" rows="3" class="w-full border rounded-lg px-3 py-2 text-sm">{{ old('catatan', $pesanan->catatan) }}</textarea>
            </div>
